...
export class AcudienteComponent implements OnInit {
  ...
  idBuscar = '';
  encontrado: Acudiente | null = null;
  buscadoId = false;
  idEdit = '';
  texto = '';
  resultados: Acudiente[] = [];
  buscadoNombre = false;
  actualizable = true;
  ...
  constructor(private servicio: AcudienteService, private fb: FormBuilder) {
  ...
  cargar() {
    ...
    this.servicio.getAcudientes().subscribe({
  ...
  private construir(v: any, isUpdate: boolean): any {
    ...
    body.email = v.email;
    body.parentNumber = (isUpdate ? 'actualizado' : v.parentNumber);
  ...
  crear() {
    ...
    this.servicio.crearAcudiente(this.construir(this.form.value, false)).subscribe({
  ...
  borrar(id: number) {
    ...
    this.servicio.eliminarAcudiente(id).subscribe({
  ...
  actualizar() {
    ...
    this.servicio.actualizarAcudiente(this.editId, this.construir(this.editForm.value, true)).subscribe({
